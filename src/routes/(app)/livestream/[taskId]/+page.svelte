<script lang="ts">
	...
	function videoAction(node: HTMLVideoElement) {
		...
		const initVideo = async () => {
			try {
				...
				if (camera.type === 'webcam' && videoStore.videoStream) {
	...
	function drawOverlay(canvas: HTMLCanvasElement, boxes: any[]) {
		...
		boxes.forEach((box) => {
			...
			if (box.label) {
				...
				ctx.fillRect(box.x - box.w / 2, box.y - box.h / 2 - 20, box.w, 20);
				ctx.fillStyle = '#ffffff';
				ctx.font = '12px Arial';
				ctx.fillText(box.label, box.x - box.w / 2 + 5, box.y - box.h / 2 - 5);
			}
		});
	}
</script>
